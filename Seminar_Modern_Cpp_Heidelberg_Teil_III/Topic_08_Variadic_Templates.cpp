...
namespace VariadicTemplatesIntro_Seminar {
    ...
    // C++ 17
    template <typename T, typename ... TArgs>   // einpacken
    void printerCpp17(T n, TArgs ... args) {         // einpacken

        std::cout << n << std::endl;

        if constexpr (sizeof... (args) > 0)
        {
            static_assert (sizeof... (args) > 0, "oben");

            std::cout << "bin hier" << std::endl;

            printerCpp17<TArgs ...>(args ...);
        }
        else
        {
            static_assert (sizeof... (args) == 0, "unten");
    ...
    std::unique_ptr<T> my_make_unique(TArgs&& ... args) {
        ...
        std::unique_ptr<T> tmp =
            std::unique_ptr<T>{ new T { std::forward<TArgs>(args) ...  } };
    ...
    void test_variadic_seminar() {
        ...
        std::unique_ptr<Unknown> ptr3 =
            my_make_unique<Unknown>(1, 2, 3);
    }
}
...
{
    VariadicTemplatesIntro_Seminar::test_variadic_seminar();
    return;
